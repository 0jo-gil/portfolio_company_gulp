...
$(window).scroll(function(){
    ...
    if(posY > brand_posY && posY < news_posY - news_ht){
        ...
        brand_counter(0, 57, 1000, posY);
        brand_counter(1, 5, 2000, posY);
        brand_counter(2, 60, 1500, posY);
...
function brand_counter(index, num, time, posY){
    let item = $(".brand_content_wrap").eq(index).find(".brand_num");
    let current_num = item.text();
    let counter_num = num - current_num;
    let interval = time / counter_num;
    let isOn = $(".brand_content_wrap").eq(index).hasClass("on");
    let brand_timer;
    brand_scroll = true;
